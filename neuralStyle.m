...
%% intialize 
addpath([cd '/matconvnet-1.0-beta16'])
addpath([cd '/functions'])
run matconvnet-1.0-beta16/matlab/vl_setupnn.m

%% set parameters
content = 'images/goldengate.jpg';
...
%% load network
if (~exist('net'))
    net = load('/Users/ignacio/Documents/CNN/NeuralStyle/models/imagenet-vgg-verydeep-19.mat');
end
...
net2=struct('layers','','classes',net.classes,'normalization', net.normalization);
j=1;
contentLayersIdx = [];
styleLayersIdx = [];
for i=1:N
...
figure(1); plot(Ldata)
figure(2); imshow(uint8(deprocess(xorig)),[]); title('xorig');
figure(3); imshow(uint8(deprocess(x)),[]); title('x');
